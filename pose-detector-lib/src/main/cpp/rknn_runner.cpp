...
struct RknnHolder {
  ...
  std::vector<int8_t> int8_input_buffer;
...
Java_com_yolo_pose_detector_RknnRunner_nativeRunPixels(JNIEnv *env,
  ...
  if (input_type == RKNN_TENSOR_FLOAT16) {
    ...
    // INT8 quantized input: apply affine quantization using input_attr.(scale,zp)
    // q = round(v / scale + zp), clamp to [-128, 127]
    const float scale = holder->input_attr.scale;
    const int zp = holder->input_attr.zp;
    const bool has_affine =
        holder->input_attr.qnt_type != RKNN_TENSOR_QNT_NONE && scale > 0.0f;

    int8_t *dst = nullptr;
    if (zero_copy_possible && num_elements <= holder->input_tensor_bytes) {
      dst = static_cast<int8_t *>(holder->input_mem->virt_addr);
    } else {
      if (holder->int8_input_buffer.size() != num_elements) {
        holder->int8_input_buffer.resize(num_elements);
      }
      dst = holder->int8_input_buffer.data();
    }

    auto quantize = [&](uint8_t v) -> int8_t {
      int q = 0;
      if (has_affine) {
        q = static_cast<int>(std::lround(static_cast<float>(v) / scale +
                                         static_cast<float>(zp)));
      } else {
        // Fallback mapping: [0,255] -> [-128,127]
        q = static_cast<int>(v) - 128;
      }
    ...
    for (int i = 0; i < pixel_count; ++i) {
      jint p = pixel_ptr[i];
      dst[i * 3 + 0] = quantize(static_cast<uint8_t>((p >> 16) & 0xFF)); // R
      dst[i * 3 + 1] = quantize(static_cast<uint8_t>((p >> 8) & 0xFF));  // G
      dst[i * 3 + 2] = quantize(static_cast<uint8_t>(p & 0xFF));         // B
    }
    input_buf = dst;
    input_size_bytes = num_elements;
  } else {
...
Java_com_yolo_pose_detector_RknnRunner_nativeRunBitmapWithNms(
  ...
  if (!rga_success) {
    ...
    if (input_type == RKNN_TENSOR_FLOAT16) {
      ...
      const int8_t pad = quantize(0);
      const uint8_t pad_u8 = static_cast<uint8_t>(pad);
      ...
      if (zero_copy_possible) {
        ...
        size_t valid_row_bytes = static_cast<size_t>(copy_width) * 3;
        for (int y = 0; y < copy_height; ++y) {
          int8_t *dst_row = dst + static_cast<size_t>(y) * dst_row_bytes;
          uint8_t *src_pixel = src_rows + static_cast<size_t>(y) * info.stride;
          for (int x = 0; x < copy_width; ++x) {
            size_t base = static_cast<size_t>(x) * 3;
            dst_row[base + 0] = quantize(src_pixel[2]); // R
            dst_row[base + 1] = quantize(src_pixel[1]); // G
            dst_row[base + 2] = quantize(src_pixel[0]); // B
            src_pixel += 4;
          }
          if (dst_row_bytes > valid_row_bytes) {
            std::memset(reinterpret_cast<void *>(dst_row + valid_row_bytes),
                        pad_u8, dst_row_bytes - valid_row_bytes);
          }
        }
        if (dst_h_stride > copy_height) {
          std::memset(reinterpret_cast<void *>(
                          dst + static_cast<size_t>(copy_height) * dst_row_bytes),
                      pad_u8, (dst_h_stride - copy_height) * dst_row_bytes);
        }
        input_size_bytes = stride_bytes;
      } else {
        size_t dst_idx = 0;
        for (int y = 0; y < info.height; ++y) {
          ...
          for (int x = 0; x < info.width; ++x) {
            ...
            dst[dst_idx++] = quantize(r);
            dst[dst_idx++] = quantize(g);
            dst[dst_idx++] = quantize(b);
      ...
      ThrowIllegalState(env, "Unsupported input type for bitmap input: " +
                                 std::to_string(input_type));
      return nullptr;
    }
