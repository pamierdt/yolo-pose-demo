...
class PoseLandmarkerHelper(
    ...
    fun setupPoseLandmarker() {
        ...
        val modelName = when (currentModel) {
            ...
            MODEL_YOLO8_RKNN_INT8 -> MODEL_FILE_YOLO8_RKNN_INT8
            MODEL_YOLO8_RKNN_HYBRID -> MODEL_FILE_YOLO8_RKNN_HYBRID
            MODEL_YOLO11_TFLITE -> MODEL_FILE_YOLO11_TFLITE
            MODEL_YOLO11_RKNN -> MODEL_FILE_YOLO11_RKNN
            MODEL_YOLO8_RKNN_CONSERVATIVE -> MODEL_FILE_YOLO8_RKNN_CONSERVATIVE
            MODEL_YOLO8_RKNN_MIXED -> MODEL_FILE_YOLO8_RKNN_MIXED
    ...
    fun runPoseEstimation(bitmap: Bitmap, validWidth: Int = bitmap.width, validHeight: Int = bitmap.height): ResultBundle? {
        ...
        if (rknnRunner != null) {
            ...
            val serializedOutput = try {
                runner.runBitmapWithNms(
                    inputBitmap,
                    minPoseDetectionConfidence,
                    minPosePresenceConfidence
                )
            } catch (t: Throwable) {
                // 避免 native 异常直接导致线程崩溃（例如旧版 so 不支持 INT8 输入）
                Log.e(TAG, "RKNN runBitmapWithNms failed: ${t.message}", t)
                poseLandmarkerHelperListener?.onError(
                    "RKNN 推理失败：${t.message ?: "unknown"}。请确认已重新编译并打包最新的 pose_detector so。"
                )
                return null
            }
    ...
    companion object {
        ...
        const val MODEL_YOLO8_RKNN_INT8 = 4
        const val MODEL_YOLO8_RKNN_HYBRID = 5
        const val MODEL_YOLO8_RKNN_MIXED = 6
        const val MODEL_YOLO8_RKNN_CONSERVATIVE = 7
